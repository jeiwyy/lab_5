...
Далее программа использует рекурсивный алгоритм Евклида для вычисления НОД: она находит остаток от деления первого числа на второе, а затем рекурсивно вызывает сама себя, передавая на место первого аргумента второе число, а на место второго — полученный остаток. 
...
Далее программа просит пользователя ввести одну цифру для поиска. 
...
В итоге программа находит единственную комбинацию, которая удовлетворяет всем заявленным правилам, и выводит результат на экран в формате `Переменная:Значение`, где 1 означает присутствие, а 0 — отсутствие.
